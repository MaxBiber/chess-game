...
export class ChessBoardComponent {
  ...
  private unmarkingPreviouslySelectedAndSafeSquares(): void {
    ...
    if (this.isPromotionActive) {
      ((this.isPromotionActive = false), (this.promotedPiece = null));
  ...
  private placingPiece(newX: number, newY: number): void {
    ...
    if (shouldOpenPromotionDialog) {
      ...
      // because now we wait for player to choose  piece
      return;
    }
  ...
  private isWrongPieceSelected(piece: FENChar): boolean {
    const isWritePieceSelected: boolean = piece === piece.toUpperCase();
    return (
